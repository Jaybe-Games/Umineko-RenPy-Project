...
define AUDIO_MOVE = THIS_PATH + AUDIO_PATH + 'umise_1001.ogg'
define AUDIO_CAPTURE = THIS_PATH + AUDIO_PATH + 'umise_036.ogg'
define AUDIO_PROMOTION = THIS_PATH + AUDIO_PATH + 'umise_059.ogg'
define AUDIO_CHECK = THIS_PATH + AUDIO_PATH + 'umise_1006.ogg'
define AUDIO_CHECKMATE = THIS_PATH + AUDIO_PATH + 'umise_054.ogg'
define AUDIO_DRAW = THIS_PATH + AUDIO_PATH + 'umise_037.ogg' # used for resign, stalemate, threefold, fifty-move
...
style promotion_piece_text is text:
    ...
    color '#ffffff' # gray
    hover_color '#ff5e00' # darker gray
...
screen chess(fen, player_color, depth):
    ...
    add "images/backgrounds/different_space_1a.png" at bgani
    add "images/sprites/bea/a11/defo2/0.png" at rightedge
    add "images/sprites/but/b11/warai2/0.png" at leftedge
    add partObj
    ...
    fixed xpos 20 ypos 80 spacing 40:
        vbox:
            showif chess_displayable.whose_turn == chess.WHITE:
                text 'Am Zug: White' style 'game_status_text'
            else:
                text 'Am Zug: Black' style 'game_status_text'
            
            showif chess_displayable.game_status == CHECKMATE:
                text 'Schachmatt' style 'game_status_text'
                ...
                text 'Matt' style 'game_status_text'
            # no need to display DRAW or RESIGN as they immediately return

            null height 50

            text 'Letzte Spielzüge' style 'game_status_text' xalign 0.5
            for move in chess_displayable.history:
                text move.uci() style 'game_status_text' xalign 0.5

    # left bottom
    fixed xpos 50 ypos 950:
        vbox:
            hbox spacing 5:
                text 'Aufgeben' color COLOR_WHITE yalign 0.5
                textbutton '⚐' action [Play('sound', AUDIO_DRAW), Return(not chess_displayable.whose_turn)] style 'control_button' yalign 0.5

    # middle panel for chess displayable
    fixed xpos 600 ypos 200:
...
init python:
    ...
    if renpy.android:
        ...
        stockfish_bin = 'stockfish-windows-x86-64-avx2.exe'
    else:
